...
# =========================================
# 2-layer NN
# 784 input layer neurons, 1000 hidden layer neurons, 10 output layer neurons
# ReLU activation in hidden layer, Softmax function on output layer
# Cross-entropy loss function used
# Implemented mini-batch gradient descent algorithm 
# Vanilla update used for parameter update
...
for train_index, valid_index in kf.split(train_data):
	...
	y_train = train[:, -1]					# select last column which is column containing labels (54000 X 1)
	...
	X_batches = np.split(X_train, 1800)			# split training set into 1800 batches with 30 records in each batch
	y_batches = np.split(y_train, 1800)			# split training set into 1800 batches with 30 records in each batch
	...
	print "iteration: ", folds_counter, " training accuracy: ", mean
	...
	print "iteration: ", folds_counter, " validation accuracy: ", mean
